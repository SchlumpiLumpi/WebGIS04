...
let sampleButton = document.getElementById("chooseRandom")
/*+++++++++++++++++++++++++++
        Classes
++++++++++++++++++++++++++++++*/
...
function file_upload() {
    ...
    if (filetype == "json" || filetype == "geojson" || filetype=="JSON" || filetype=="geoJSON" || filetype=="GEOJSON") {
        ...
        reader.onload = function () {
            ...
            "<input id='sampleNumber' type='number' class='form-control' placeholder='0' min='1' max='"+cities.features.length+"'>"
...
//Random cities 
function randomCitieSample() {
    
    if (validFile == true) {
        sampleNumber = parseInt(document.getElementById("sampleNumber").value)
        let randomCities = turf.sample(cities, sampleNumber)
        console.log("random Selection from Cities", randomCities)
        drawCityMarkers(randomCities,"Random Selection")
        console.log("Overlay changed: ",layerControl.getOverlays())
...
sampleButton.addEventListener("click", randomCitieSample)
